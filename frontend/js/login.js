...
/* ─── Default tab from URL (?tab=register|trainer or hash #register|#trainer) ─── */
var urlParams = new URLSearchParams(window.location.search);
var initialTab = urlParams.get('tab');
if (window.location.hash === '#register' || initialTab === 'register') {
    if (registerTab && registerForm) switchAuthTab(registerTab, registerForm);
} else if (window.location.hash === '#trainer' || initialTab === 'trainer') {
    if (trainerRegTab && trainerRegForm) switchAuthTab(trainerRegTab, trainerRegForm);
}

/**
 * Returns a safe relative redirect target from the `next` query param,
 * or null if it's missing / unsafe (absolute URLs, javascript:, etc.).
 */
function getSafeNextUrl() {
    var raw = urlParams.get('next');
    if (!raw) return null;
    var lower = raw.toLowerCase();
    if (lower.indexOf('//') === 0) return null;
    if (lower.indexOf('http:') === 0 || lower.indexOf('https:') === 0) return null;
    if (lower.indexOf('javascript:') === 0 || lower.indexOf('data:') === 0) return null;
    return raw;
}
...
function routeAfterLogin(user) {
    ...
    var nextUrl = getSafeNextUrl();   // honoured only for regular members
    ...
    db.collection('admins').doc(uid).get()
        ...
        .then(function(tDoc) {
            ...
            window.location.href = nextUrl || 'member.html';
        })
        .catch(function(err) {
            console.error(err);
            if (authAlert) {
